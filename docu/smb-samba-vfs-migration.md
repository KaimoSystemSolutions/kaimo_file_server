...
| **2b — Open/Path ACL** ⚠️ | VFS `create_file` hook → complete raw SMB access mask → gRPC `AuthorizeOpen` → attenuated specific mask returned to Samba; `readdir` uses listing-only checks and the sidecar caches decisions plus granted masks | All Kaimo permission bits, generic rights, `MAXIMUM_ALLOWED`, traversal, and file-vs-directory creation are implemented and managed-tested; native Samba runtime verification of P0-03 remains pending |
